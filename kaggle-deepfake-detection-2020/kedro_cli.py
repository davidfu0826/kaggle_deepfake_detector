...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/kaggle_deepfake_detection_2020")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=kaggle_deepfake_detection_2020"]
    )
    ...
    call(
        [
            ...
            "-o",
            "docs/source",
            "src/kaggle_deepfake_detection_2020",
        ]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "Kaggle Deepfake Detection 2020"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "kaggle_deepfake_detection_2020"
